  ...
  return (
    <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border-2 border-gray-100 dark:border-slate-800 shadow-sm hover:border-purple-100 dark:hover:border-purple-900 transition-all duration-200 flex flex-col h-full">
      <div className="flex justify-between items-start mb-6">
        ...
        <a href={company.careerUrl} target="_blank" rel="noopener noreferrer" className="text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 transition-colors p-2 hover:bg-purple-50 dark:hover:bg-purple-900/10 rounded-xl">
          <ExternalLink className="h-5 w-5" />
        </a>
      </div>
      <div className="flex items-center gap-3 mb-3">
        <h2 className="text-2xl font-black text-gray-900 dark:text-slate-100">{company.name}</h2>
        {activeCount > 0 && (
          <span className="bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-400 px-2 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-wider">
            {activeCount} active roles
          </span>
        )}
      </div>
      <p className="text-gray-500 dark:text-slate-400 text-sm leading-relaxed mb-6 flex-grow">
        {company.description}
      </p>
      
      <div className="space-y-4 pt-4 border-t border-gray-50 dark:border-slate-800 mt-auto">
        <div>
          <h3 className="text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-2">
             <Globe className="h-3 w-3" />
             Active Regions
          </h3>
          <div className="flex flex-wrap gap-2">
            {company.regions.length > 0 ? company.regions.map(r => (
              <span key={r} className="inline-flex items-center gap-1.5 bg-gray-50 dark:bg-slate-800 text-gray-700 dark:text-slate-300 px-3 py-1 rounded-lg text-xs font-bold border border-gray-100 dark:border-slate-700">
                <span>{REGION_FLAGS[r] || '📍'}</span>{r}
              </span>
            )) : <span className="text-xs text-gray-400 dark:text-slate-500 italic">No specific regions tracked</span>}
          </div>
        </div>
      ...
      <button 
        ...
        className={`mt-8 w-full py-3 rounded-2xl text-sm font-black transition-colors flex items-center justify-center gap-2 ${
          activeCount > 0 
          ? 'bg-purple-50 dark:bg-purple-900/10 hover:bg-purple-100 dark:hover:bg-purple-900/20 text-purple-700 dark:text-purple-400' 
